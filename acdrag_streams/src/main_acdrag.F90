...
!$ACC& IF (TRIM (CLMETHOD) == 'openaccsinglecolumn' .OR. TRIM (CLMETHOD) == 'openaccmanyblocks'& 
!$ACC& .OR. TRIM(CLMETHOD)=='openaccmanyblocks_str')


IF (TRIM (CLMETHOD) == 'openaccsinglecolumn' .OR. TRIM (CLMETHOD) == 'openaccmanyblocks' .OR. TRIM(CLMETHOD)=='openaccmanyblocks_str') THEN
...
!$OMP END PARALLEL DO
  ...
  ELSEIF (TRIM (CLMETHOD) == 'openaccmanyblocks_str') THEN

      LLACC=.TRUE.
      YLOFFSET=FXTRAN_ACDC_STACK(0,0,0,0)
      CALL ACDRAG_MANYBLOCKS_STR(YDCST, YDML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
      & ZZAPRS, ZZAPRSF,  &
      & ZZDELP, ZZNBVNO,  &
      & ZZRDELP, ZZU, ZZV,  &
      & ZZRCORI, ZZGETRL, ZZGWDCS,  &
