...
DO JLEV=KLEV-1, KTDIA,-1
  ...
  IF (JLEV==KLEV-1) THEN
    LLTEST=.TRUE.
  ELSE
    LLTEST=ZWGHT (JLON, JLEV+1)>0.0_JPRB
  ENDIF
...
ZUSTAR=ZSUMU *FXTRAN_ACDC_BR_COS (2.0_JPRB*PVRLDI (JLON)&
&)+ZSUMV *FXTRAN_ACDC_BR_SIN (2.0_JPRB*PVRLDI (JLON))
ZVSTAR=ZSUMU *FXTRAN_ACDC_BR_SIN (2.0_JPRB*PVRLDI (JLON)&
&)-ZSUMV *FXTRAN_ACDC_BR_COS (2.0_JPRB*PVRLDI (JLON))
...
ZUSR =SQRT (FXTRAN_ACDC_BR_POW(ZUSUR , 2)+FXTRAN_ACDC_BR_POW(ZVSUR , 2))
ZFACS =ZNFNO (JLON, KLEV)/FXTRAN_ACDC_BR_POW(ZSUSR , 3)
ZRAPP (JLON, KLEV)=1.0_JPRB
ZSTUS =YDML_PHY_MF%YRPHY0%GWDSE*FXTRAN_ACDC_BR_POW(PGWDCS&
& (JLON), 2)*ZNFNO (JLON, KLEV)*PGETRL (JLON)*ZUSUR 
ZSTVS =YDML_PHY_MF%YRPHY0%GWDSE*FXTRAN_ACDC_BR_POW(PGWDCS&
& (JLON), 2)*ZNFNO (JLON, KLEV)*PGETRL (JLON)*ZVSUR 
ZTST1 =1.0_JPRB
...
ZREF =1.0_JPRB
ZTUNE=YDML_PHY_MF%YRPHY0%GWDBC*FXTRAN_ACDC_BR_POW((ZUSR **2/ZSUSR ), 2)
...
DO JLEV=KLEV-1, KTDIA,-1
  ...
  ZPRRAP=ZFACS *FXTRAN_ACDC_BR_POW(ZAUSR, 3)/MAX (ZEPS3, ZNFNO (JLON, JLEV))
...
ZSTM =1.0_JPRB/SQRT (1.0_JPRB+MAX (0.0_JPRB, SIGN (1.0_JPRB, ZPR1 -ZPR2 -ZEPS4))*YDML_PHY_MF%YRPHY0&
&%GWDAMP*(2.0_JPRB*FXTRAN_ACDC_BR_SIN (MIN (ZARGLI, ZSUM ))+YDML_PHY_MF%YRPHY0%GWDAMP))
...
DO JLEV=KTDIA, KLEV
  ...
  ELSE
    ZRZR=SQRT (ZZBB /(1.0_JPRB+FXTRAN_ACDC_BR_POW(ZRZ, 2)*ZZBB *(1.0_JPRB-ZZBB )))
  ENDIF

  ZRAPP (JLON, JLEV)=ZRAPP (JLON, JLEV)+ZZCR *(ZZAA *SQRT (MAX (0.0_JPRB,FXTRAN_ACDC_BR_POW&
  &((1.0_JPRB-ZRZ*ZRZR), 3)/(1.0_JPRB+ZRZ*YDML_PHY_MF%YRPHY0%GWDPROF*ZZBB ))))
...
ZSUMD =0.5_JPRB*ZWGHT (JLON, KTDIA)*ZIPOI (JLON, KTDIA)*(ZRAPP (JLON, KTDIA)-ZRAPP (JLON, KTDIA-1))
...
ZSUMD =ZSUMD *SQRT (FXTRAN_ACDC_BR_POW(ZSTUS , 2)+FXTRAN_ACDC_BR_POW(ZSTVS , 2))
